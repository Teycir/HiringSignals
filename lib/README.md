...
| `kv/snapshot-mirror.ts` | KV-backed mirror of `d1/snapshot-store.ts`, same `(domain, entity_key)` key shape and no-TTL/no-expiry philosophy. Exists because a D1 snapshot row is still a D1 read, vulnerable to an account-wide D1 quota outage; this gives a read path a second, entirely separate-quota copy to fall back to so it survives a full D1 outage, not just a cheaper D1 query. Always written best-effort, alongside (never instead of) the real D1 write. | `@cloudflare/workers-types` |
