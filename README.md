...
- **Signal scoring (v3)**: Priority score (0–100) = `0.35*freshness + 0.25*volume + 0.20*acceleration + 0.10*breadth + 0.10*classification_confidence`; decays over time with no new evidence. v3 (2026-08-19) special-cases acceleration for company+role pairs with no prior 56-day baseline (`newInPrior56Days=0`): those are scored on an absolute last-14-day scale instead of the spec's relative-rate comparison, which had saturated every such case to 1.0 (11 of 14 companies on the live dataset)
- **Score-distribution stats**: `GET /api/v1/signals/stats` returns count, min/max/mean/median/p25/p75 of signal scores plus per-signal-type and per-role-category breakdown counts, over the same filter surface as the feed
...
pnpm --filter @hiring-signals/web dev     # Next.js dev server (http://localhost:3000)
```
