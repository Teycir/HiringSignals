...
async function main() {
  ...
  console.log("first():", JSON.stringify(row));
  ...
  const runResult = await client.run(
    ...
    ["smoke-test-id-1", "smoke-test-slug-1", "Smoke Test Co", "2020-01-01T00:00:00.000Z", "2020-01-01T00:00:00.000Z"],
  );
  console.log("run() insert:", JSON.stringify(runResult));

  const persisted = await client.first("SELECT * FROM companies WHERE id = ?", ["smoke-test-id-1"]);
  console.log("read-back:", JSON.stringify(persisted));
