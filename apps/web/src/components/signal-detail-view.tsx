"use client";
// Extracted from app/signals/[signalId]/page.tsx (SEO fix, 2026-08-17):
// that file needed to become a server component so it could export
// generateMetadata (a signal's real headline/company in <title>/
// <meta description>, instead of the root layout's generic fallback
// every crawler saw before this split -- generateMetadata cannot live
// in a "use client" file). The actual fetch-on-mount + loading/error/
// not-found rendering logic is unchanged from before the split; it just
// lives here now so page.tsx can stay a server component. Client-fetch,
// not a server component reading fetchSignalDetail directly: mirrors
// signals-view.tsx's established pattern (useEffect + api-client.ts
// call) rather than introducing a second fetch strategy into the
// codebase. No useSearchParams() here (route param, not a query
// string), so unlike signals-view.tsx this needs no Suspense boundary --
// signalId comes in as a plain prop from the server page.tsx.
