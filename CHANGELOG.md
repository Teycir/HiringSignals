...
- **`source-health.mjs` couldn't detect a source stuck at `status='running'` (ROADMAP.md G.3/spec §16.3.6):** `deriveStatus()` only branched on `enabled`/`consecutive_failures`/`last_run_status === 'failed_final'`, so a run that crashed mid-invocation (the Cloudflare subrequest-cap bug fixed the same day, see `ingest-consumer.ts`) and never resolved to a terminal state showed as "healthy, 0 failures" instead of the real problem — `openai` had 277 such rows invisible to this script despite `ingestion-metrics.mjs` correctly flagging a 9.1% success rate on the same underlying data. Added a `running_minutes` column (correlated subquery on the existing latest-run lookup) and a `STALE_RUNNING_MINUTES = 90` threshold with a new `"stuck"` status, checked ahead of the existing degraded/failed branches. Verified live against local D1 (16 sources, all healthy) and production D1 (10 sources) — `openai` now correctly shows healthy post-fix, no source currently flagged stuck.
