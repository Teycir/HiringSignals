...
- **`/trends` returning "Something went wrong processing the request" for real user traffic, root-caused to D1's free-tier daily row-read quota being exhausted (again — same condition as an earlier session).** Investigation found and fixed two separate, stacked problems before reaching the real root cause: (1) `apps/web`'s live deployed build was stale — `.open-next`/`.wrangler` on disk predated the `/trends` route and `NEXT_PUBLIC_API_BASE_URL` entirely, fixed with a fresh `pnpm deploy` from current source; (2) after that fix, the same request still intermittently 500'd — confirmed via `wrangler tail` that D1's daily row-read quota was exhausted, and `readTrendsSnapshots`'s live-D1 read was the only path to serve `snapshots_current`, with no fallback if that read itself threw.

- **`trends.ts`/`signals.ts` snapshot reads had no fallback independent of D1.** `snapshots_current` (the daily-cron-captured table both routes already read from — see `snapshot-persistence-plan.md`) is still a D1 row, so an account-wide quota exhaustion could take out the snapshot read itself, not just a live-table-sized query. Added a KV mirror: `handleSnapshotCapture` (`reconciliation.ts`) now writes every snapshot to both D1 and a no-TTL KV key immediately after each successful D1 write, always best-effort so a mirror-write failure can never fail the capture pass. New generic module `lib/kv/snapshot-mirror.ts` (same `(domain, entity_key)` keying as `lib/d1/snapshot-store.ts`, its D1 counterpart), typed wrappers added to `packages/db/src/snapshot-repo.ts`. `trends.ts` falls back to the KV mirror when its D1 snapshot read throws. `signals.ts`'s existing live-query → D1-snapshot fallback chain gained a third rung, the same KV mirror, for when the D1 snapshot read also fails — previously it had no fallback beyond the D1 snapshot at all. See `snapshot-persistence-plan.md` §10 for the full design writeup.

- **`reconciliation.ts` had a scope bug that would have broken the trends KV write outright.** The trends-mirror write inside `handleSnapshotCapture` referenced `env.CACHE`, but `env` was never in that function's scope — its second parameter is named `cache`. Caught before deploy via `pnpm -r typecheck` + `wrangler deploy --dry-run`, not in production.

### Changed (2026-09-03)

- **`trends.ts` and `signals.ts` deliberately diverge on read order — trends stays snapshot-first, signals stays live-first — and this is now documented in both files' header comments plus `snapshot-persistence-plan.md` §6/§10 and this README's new "Read-path resilience" section, so the asymmetry reads as a priced decision, not an inconsistency.** `getHiringTrends` (the live query `trends.ts` used pre-`snapshot-persistence-plan.md`) unconditionally scans every `jobs` row matching the requested roles before `limit` applies — the exact unbounded query that caused the original D1 quota exhaustion — so it stays confined to the once-a-day cron, off request traffic entirely; trends' own `since`/7-day-window semantics are already daily-granularity by design, so reading the snapshot by default costs no fidelity there. `listSignals`, by contrast, is a bounded, indexed `WHERE ... LIMIT` read against `signals` — cheap regardless of request volume — and only the live path supports cursor-based pagination, full SQL `q` search, and the semantic/Vectorize hybrid leg; the snapshot fallback (`filterSnapshotItems`) is a deliberately degraded single-page/substring-search view, not a full reimplementation. Live-first on signals was evaluated and rejected for trends specifically because of this cost/fidelity asymmetry, not overlooked.
...
- **CI/CD quality gate enforcement.** Added git pre-push hook that runs typecheck and lint locally before allowing any push to remote, ensuring code quality gates are enforced before code reaches the CI pipeline. Setup script `scripts/setup-pre-push-hook.sh` provided for easy installation. Simplified GitHub Actions CI workflow (`.github/workflows/ci.yml`) to run typecheck and lint on every push to main and pull requests, providing a remote quality gate. Updated `.github/DEPLOYMENT.md` with CI/CD documentation.
