...
- **Milestone I — Semantic search (Workers AI + Vectorize):** Scoped in `ROADMAP.md`. I.1 (index + bindings) and I.2 (embedding write path — jobs embedded and upserted into Vectorize at ingest time, best-effort/non-blocking) done. I.3 (query-side hybrid search) partially done: semantic-search service and keyword/semantic merge logic implemented in `apps/api/src/services/semantic-search.ts` and `packages/domain/src/signal-search-merge.ts`, but query-side hybrid search is not yet called from the live `GET /api/v1/signals` route. I.4 (search UI) and I.5 (classification assist) pending.
