...
- **Automated deployment pipeline via GitHub Actions.** New `.github/workflows/deploy.yml` automatically deploys `apps/api` and `apps/web` to Cloudflare Workers after CI passes on main branch pushes, with manual trigger support via `workflow_dispatch`. Deployment only runs after successful CI completion (typecheck, lint, and all tests), ensuring production quality gates before deployment. Added `.github/DEPLOYMENT.md` with setup instructions for the required `CLOUDFLARE_API_TOKEN` GitHub secret and security best practices.
